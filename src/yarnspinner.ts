...
export async function init(
  variableStorage: IVariableStorage,
): Promise<IYarnSpinnerRuntimeInfo> {
...
class Dialogue implements IDialogue {
  ...
  registerFunction(functionDefinition: IFunctionDefinition): Promise<void> {
    return (async () => {
      window.yarnFunctions[functionDefinition.name] =
        functionDefinition.function;

      this.compilation = await this.dotNetDialogue.invokeMethodAsync(
        "RegisterFunction",
        functionDefinition,
      );
    })();
  ...
  startDialogue(nodeName: string): Promise<void> {
    if (this.compilation == null) {
      this.onError(
        new Error(
          "Can't start dialogue: no Yarn source code has been compiled yet",
        ),
      );
      return;
    }
    ...
    return (async () => {
      try {
        ...
        while (true) {
          var events = (await this.dotNetDialogue.invokeMethodAsync(
            "Continue",
          )) as any[];
          ...
          for (let event of events) {
            if (event.type === "line") {
              ...
              await this.dotNetDialogue.invokeMethodAsync(
                "SetSelectedOption",
                selectedItem,
              );
