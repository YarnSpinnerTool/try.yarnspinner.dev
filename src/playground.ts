...
export async function load(
  initialContentName: string = "default",
  ...
  if (titleField) {
    titleField.addEventListener("input", () =>
      window.localStorage.setItem("title", titleField.value)
    );
    if (title) {
  ...
  if (authorField) {
    authorField.addEventListener("input", () =>
      window.localStorage.setItem("author", authorField.value)
    );
    if (author) {
  ...
  const versionValueField = document.getElementById(
    "yarn-spinner-version-value"
  );
  ...
  const generateBookButton = document.getElementById(
    "generate-book"
  ) as HTMLButtonElement;
  const pdfGenerationAlert = document.getElementById(
    "pdf-generate-alert"
  ) as HTMLDivElement;
  const pdfEmbedElement = document.getElementById(
    "book-embed"
  ) as HTMLIFrameElement;
  const pdgGenerationSpinner = document.getElementById(
    "pdf-generate-spinner"
  ) as HTMLDivElement;
  ...
  const updatePDFPreviewClicked = async () => {
    ...
    try {
      if (errorsExist) {
        throw new Error(
          "Can't create PDF, because errors exist in your Yarn script. Fix them and try again."
        );
      }
  ...
  const dispatchPDFGenerationJob = async () => {
    ...
    const author =
      authorElement?.value || authorElement?.placeholder || "Author";
    ...
    var data =
      await schemas.pDFGenerationReponseSchema.parseAsync(responseJSON);
    ...
    while (pollCount < maxPolls) {
      ...
      const jobStatusResponse = await fetch(
        `${pdfPollEndpoint}?${q.toString()}`,
        {
          method: "GET",
        }
      );

      var status = await schemas.pDFGenerationReponseSchema.parseAsync(
        JSON.parse(await jobStatusResponse.text())
      );
  ...
  downloadButton.addEventListener("click", async () => {
    ...
    dispatchPDFGenerationJob()
      .then(async (pdfLocation) => {
        ...
        if (pdfResponse.ok) {
          ...
          throw new Error(
            "PDF generation reported as successful, but download failed"
          );
        }
      })
      .catch((err) => {
